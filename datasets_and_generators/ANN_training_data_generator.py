...
range_size = 1000  # global range variable
...
for i in range(range_size):
    ...
    for j in range(len(grids)):
        ...
        try:
            raysWithBuildings = gp.sjoin(rays, polys[0], op="intersects")
        except:
        ...
        #     json.dump(list(grids[j]['count']), outfile)
        if i < 10:
            ax = x.plot()
    ...
    #concatenate to file
    append_to_json(grabANNdata(grids), '../datasets_and_generators/ANN_trainingdata_MASTER.json')

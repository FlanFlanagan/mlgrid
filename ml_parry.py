...
def main():
    ...
    for i in range(range_size):
        ...
        x_max = maxes[i][0]
        y_max = maxes[i][1]
        xs = np.linspace(0, x_max, 20)
        ys = np.linspace(0, y_max, 20)
    ...
    for i in range(len(grids)):
        ...
        rays = pbf.build_lines_from_point(samplePoints, 10, 5)
    ...
    for i in range(len(grids)):
        ...
        for j in input_list[i].index:
            if grids[i].at[j, 'count'] != 0:
                input_list[i].at[j, 'count'] = 0  # wall
            else:
                output_list[i].at[j, 'count'] = -1  # street
                input_list[i].at[j, 'count'] = -1  # street
